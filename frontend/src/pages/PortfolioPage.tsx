          ...
          <div className="watchlist-list">
            {holdings.map((holding) => (
              <article className="watchlist-item" key={holding.holdingId}>
                ...
                <div className="watchlist-main"><strong>{holding.name}</strong><small>当前：{holding.currentPrice ?? "暂无数据"}　<span className={Number(holding.changePercent) > 0 ? "quote-up" : Number(holding.changePercent) < 0 ? "quote-down" : "quote-flat"}>{holding.changePercent ? `${holding.changePercent}%` : "暂无数据"}</span></small>{holding.profileStatus === "PENDING" ? <small className="profile-pending">正在建立股票画像</small> : null}</div>

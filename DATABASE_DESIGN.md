...
`schema_migrations` 是迁移系统内部表，保存迁移版本、校验标识与应用时间。当前已定义 `001`（数据库核心）至 `016`（多 Provider 配置与逐证券 AI 报告）；迁移重复执行不会重新执行已应用版本，已应用迁移的校验标识不匹配会阻止继续启动。
...
- `ai_provider_settings` 由迁移 `015` 初始化 DeepSeek、腾讯混元、豆包的模型、优先级与启用状态。运行时只选择优先级最高且同时“已开启/已配置”的一个 Provider，绝不自动并行调用多个模型。

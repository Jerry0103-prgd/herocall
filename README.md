...
项目已完成基础工程初始化、金融领域数据库、Portfolio Engine、Market Data Engine、Dashboard、我的关注、设置、市场情报、AI复盘、市场雷达和首次启动向导：包含 Tauri 2、React、TypeScript、Rust、SQLite 迁移框架，以及 Tushare/东方财富/腾讯 Adapter、行情规范化与用户主动保存的市场快照。市场情报按关注标的聚合东方财富公告（A 级官方确认）与公开财经快讯（B 级权威媒体），保存来源、原文、发布时间与抓取时间；社区观点和传闻会明确隔离，不得作为客观事实。没有可验证的数据时显示“暂无数据”或“未确认”。AI复盘针对每只关注标的独立生成可审计的 Research Engine V2 报告：核心驱动、市场交易逻辑、多空博弈、未来催化、风险因素、研究评分与股票画像均须引用冻结 Evidence Context。新关注标的画像先明确为“正在建立股票画像”，不会由代码或名称推断公司事实。支持 DeepSeek、腾讯混元（TokenHub）、豆包的独立 Keychain 配置；每次只调用优先级最高且已开启、已配置的一个 Provider。腾讯 TokenHub 使用 OpenAI 兼容接口与 `hy3`，连接测试只请求模型列表，不会发送投研上下文。API Key 不写入 SQLite、源码、日志或前端。AI 不提供直接交易指令、目标价、收益预测或收益承诺。
